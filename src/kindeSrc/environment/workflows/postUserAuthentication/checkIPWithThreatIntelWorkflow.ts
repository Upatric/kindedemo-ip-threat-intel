...
import {
  ...
  /**
   * Updates a user property in Kinde using the Management API.
   * @param userId The user ID to update.
   * @param propertyName The name of the property to update.
   * @param propertyValue The value to set for the property.
   * @param kindeDomain The Kinde domain.
   * @param clientId The M2M client ID.
   * @param clientSecret The M2M client secret.
   */
  async function updateUserProperty(
    ...
    try {
      ...
      const formData = `grant_type=client_credentials&client_id=${encodeURIComponent(clientId)}&client_secret=${encodeURIComponent(clientSecret)}&audience=${encodeURIComponent(fullDomain)}/api&scope=${encodeURIComponent('read:user_properties update:user_properties')}`;
      ...
      const updateResponse = await fetch(`${fullDomain}/api/v1/users/${userId}/properties/${propertyName}?value=${propertyValue.toString()}`, {
        method: 'PUT',
        headers: {
      ...
      if (updateResponse.data) {
        ...
        if (updateData.error) {
          throw new Error(`Failed to update user property: ${updateData.error}`);
        }
        if (updateData.status === "error") {
          throw new Error(`Failed to update user property: ${updateData.error_id || 'Unknown error'}`);
        }
      } else if (!updateResponse.ok) {
        throw new Error(`Failed to update user property: ${updateResponse.status}`);
      }
      
      console.log(`Successfully updated user property: ${propertyName} = ${propertyValue}`);
    } catch (error) {
      console.warn(`Failed to update user property ${propertyName}:`, error);
  ...
  export default async function handlePostAuth(
    ...
    // TESTING: Hardcode abuse score for testing - OVERRIDE ANY SCORE (cache or API)
    // abuseConfidenceScore = 75; // Change this to any test score 0-100 and uncomment to use
    ...
    // Store the abuse score in the user's Kinde properties
    try {
      ...
      if (kindeDomain && clientId && clientSecret) {
        await updateUserProperty(
          ...
          abuseConfidenceScore.toString(),
          kindeDomain,
          clientId,
          clientSecret
        );
      } else {
        console.warn('Missing Kinde Management API credentials. Skipping user property update.');
      }
    } catch (propertyError) {
      console.warn('Failed to update user property with abuse score:', propertyError);
    }
  }
